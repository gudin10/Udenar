...
for ($i = 0; $i < count($datos); $i++) {
    ...
    if ($objeto->getIdTrabajador()==$idTrabajador) {
        ...
        $lista.="<td>{$objeto->getNombres()}</td><td>{$objeto->getApellidos()}</td><td>{$objeto->getEdad()}</td><td>{$objeto->getNombreSexo()}</td><td>{$objeto->getCelular()}</td>"
         ...
         $lista.='<td>';
         $lista.="<a href='principal.php?CONTENIDO=interfaces/administrador/grupoFamiliarFormulario.php&accion=Modificar&id={$objeto->getId()}&idTrabajador={$objeto->getIdTrabajador()}'>"
...
<table class="table table-sm">
    <tr><th>NOMBRES</th><th>APELLIDOS</th><th>EDAD</th><th>GENERO</th><th>CELULAR</th><th>OCUPACION</th><th>PARENTESCO</th>
        <th><a href="principal.php?CONTENIDO=interfaces/administrador/grupoFamiliarFormulario.php&accion=Adicionar&idTrabajador=<?=$trabajador->getidentificacion()?>">
...
<script type="text/javascript">
    function eliminar(id, idTrabajador){
        if(confirm('Realmente desea eliminar este registro?'))
            location='principal.php?CONTENIDO=interfaces/administrador/grupoFamiliarActualizar.php&accion=Eliminar&id='+id+'&idTrabajador='+idTrabajador;
    }
</script>
